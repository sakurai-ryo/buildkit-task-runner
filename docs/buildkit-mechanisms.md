...
## 8. 設計の考察：なぜ hacky か / いつ BuildKit が効くか

このプロジェクトの依存表現（4-4 の「捨てパスへの readonly マウントで順序辺を作る」）は、正直に言うと **hacky** だ。
その理由と、どういう時に BuildKit を使う価値があるのかを整理しておく。

### なぜ hacky になるのか

BuildKit は本来 **「1つのコンテナイメージ（= 1つの rootfs）をビルドする」**ための道具。
Dockerfile を思い浮かべると、依存はごく自然に表現される:

```dockerfile
FROM golang AS deps
RUN go mod download   # ← この層の上に
FROM deps
RUN go build          # ← 次が積まれる（前の rootfs を継承するのが自然）
```

「前の成果物の**上に**積む」＝ rootfs チェーンが依存そのもの。**データ依存と実行順序が一致**しているので、
LLB が「辺 = 入力（データ依存）」しか持たないのは理にかなっている。

一方タスクランナーがやりたいのは:

- 各タスクは**別々のイメージ**で動くかもしれない（golang と alpine が混在）
- 成果物は引き継がず、**「先に終わってさえいればいい」という純粋な順序関係**

これは BuildKit の「1 rootfs を積み上げる」モデルから外れている。だから `/.btr/deps/...` への捨てマウントで
**順序辺を“捏造”する** hack が必要になる（詳細は 4-4、[`run.AddMount`](../internal/llbgen/llbgen.go) の節）。

### もしホスト上で動けばよいなら、もっとシンプル

仮に「各タスクをホスト上でコマンド実行するだけ」でよければ、**BuildKit はほぼ不要**になる。必要なのは:

1. 依存解決（トポロジカルソート）— 自前で簡単に書ける
2. 並列実行 — goroutine + `errgroup` / channel で数行
3. コマンド実行 — `os/exec`

順序は「依存の完了を待つ」制御フローでそのまま書けるため、**順序辺を捏造する hack は消える**。
これは Make / Task / just がやっていることそのもの。外部デーモンも Docker も LLB も要らない。

| | BuildKit 版（今回） | ホスト実行版 |
|---|---|---|
| 依存の表現 | 捨てマウントで入力辺を捏造（hacky） | `<-done[dep]` で待つだけ（自然） |
| 並列 | BuildKit が自動 | goroutine / errgroup（数行） |
| キャッシュ | コンテンツアドレッサブルで自動 | **自前実装が必要**（or 無し） |
| 隔離・再現性 | コンテナで隔離、`image` 固定で再現 | ホスト依存・汚染あり |
| 依存物 | buildkitd + Docker | **Go バイナリ単体** |

### では、いつ BuildKit を使う価値があるか

ホスト実行版だと自前で大変な部分を「タダで」得たいときに、初めて元が取れる:

- **コンテンツアドレッサブルなキャッシュ**（入力が同じなら再実行しない。5-3）
- **コンテナによる隔離・環境再現性**（`image` 固定で誰の環境でも同じ結果）
- **永続キャッシュマウント**（`/go/pkg/mod` 共有など。4-7）

裏を返すと、今回の `btr` の hacky さは **「ホストで十分なことを、あえてコンテナ実行基盤の上でやっている」**ことに由来する。
学習用としてはまさにそこが面白く、「タスクランナーが欲しいだけ」なら不要なものを、あえて触って体感している。

### より素直にやるなら（発展）

hack を減らす方向の選択肢もある:

- **gateway frontend でオーケストレーション** — `client.Build`（gateway API）で、タスク間の順序は **Go 側の制御フロー**で持ち、
  BuildKit には個々のタスクだけを投げる。「順序を LLB に埋め込む」必要がなくなり、捨てマウントの hack が消える。
  本格的に作るならこれが一番きれい。
- **`llb.Merge` / `llb.Diff`** — 「順序」ではなく「並列に作って合成」を表現したい場合に素直なことがある。

今回の「LLB に順序を捏造する」方式は、**遊び／学習用の割り切り**として位置づけるのが正直なところ。
...
- gateway frontend によるオーケストレーション（8章「発展」参照）。
